...
impl NativeViewImp for HtmlView{
    fn new() -> Self {
        ...
        let div = document.create_element("div").expect("should be able to create div");

        return Self { div: div.unchecked_into() }
    }
    fn set_width(&self, width: f32) {
        let _ = self.div.style().set_property("width", &format!("{}px", width as i32));
    }
    fn set_height(&self, height: f32) {
        let _ = self.div.style().set_property(
            "height", 
            &format!("{}px", height as i32)
        );
    }
